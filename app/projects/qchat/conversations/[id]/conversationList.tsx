...
export default function ConversationList({ appId, handleClick, handleRefreshClick }) {
            ...
            <div className="flex h-full flex-col px-3 py-4 md:px-2">
                <div className="flex grow flex-row justify-between space-x-2 md:flex-col md:space-x-0 md:space-y-2">
                    ...
                    {conversations?.items?.map((item: any) => {
                        ...
                        return (
                            <div className=" flex h-[48px] text-sm grow items-center justify-center gap-2 rounded-md bg-gray-50 p-3 font-medium hover:bg-sky-100 hover:text-blue-600 md:flex-none md:justify-start md:p-2 md:px-3" key={item.id}>
                                <Button variant="link" onClick={() => handleClick(item)}>
                                    <ChatBubbleLeftRightIcon className="w-6" /> {dt.toDateString() === dtToday.toDateString() ? dt.toTimeString().slice(0, 5) : dt.toDateString()}
                                </Button>
                            </div>
                        )
                    }).reverse()}
