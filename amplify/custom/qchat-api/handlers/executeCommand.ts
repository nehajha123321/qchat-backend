...
async function getIndexedPages(
  ...
  content: any,
  responseListIndices: ListIndicesCommandOutput,
) {
  var total = 0;
  var items = responseListIndices.indices;
  if (items === null || items === undefined) return total;
  for (const item of items) {
    if (item.status === "ACTIVE") {
      //get indexed files
      var getIndex: GetIndexCommandInput = {
        indexId: item.indexId,
        applicationId: content.applicationIdQ,
      };
      var getIndexCommand = new GetIndexCommand(getIndex);
      var getIndexResponse: GetIndexCommandOutput =
        await qBusinessClient.send(getIndexCommand);
      var count =
        getIndexResponse.indexStatistics?.textDocumentStatistics
          ?.indexedTextDocumentCount || 0;
      total = total + count;
    }
  }
  return total;
